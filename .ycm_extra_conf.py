...
def LoadSystemIncludes():
    regex = re.compile(
        r'(?:\#include \<...\> search starts here\:)(?P<list>.*?)(?:End of search list)',
        re.DOTALL)
    process = []
    try:
        process = subprocess.Popen(['clang-3.9', '-v', '-E', '-x', 'c++', '-'],
...
if os.path.exists(compilation_database_folder) and os.path.exists(
...
def FlagsForFile(filename, **kwargs):
    if IsHeaderFile(filename):
        ...
        final_flags = MakeRelativePathsInFlagsAbsolute(
            flags, relative_to) + systemIncludes
